...
/**
 * Expects KEY_MAZE String extra from user, which is supposed to be a String obtained by encoding
 * a maze generated by the MazeGenerator class with the BluetoothEncoderDecoder.encodeMaze(int[][]) function
 * @author USER
 *
 */
public class FlexibleDrawViewMazeActivity extends Activity{
	//BT STARTS
	...
	Handler btHandler = new Handler(){
				...
				public void handleMessage(Message msg) {
					...
					case MESSAGE_STATE_CHANGE:
						...
						case BluetoothChatService.STATE_NONE:
							Toast.makeText(FlexibleDrawViewMazeActivity.this,getString(R.string.title_not_connected,
					...
					case MESSAGE_READ:
						...
						Toast.makeText(FlexibleDrawViewMazeActivity.this, readMessage, Toast.LENGTH_SHORT).show();
		...
		private OnClickListener endActivityOnClickListener = new OnClickListener(){
			...
			public void onClick(View v){
				FlexibleDrawViewMazeActivity.this.finish();
			}
		};
	...
	public void onCreate(Bundle savedInstanceState){
		...
		mChatService = BluetoothMediator.mChatService;//alt: have the BluetoothChatService class have a static variable maintaining a reference to a service onse it's been started
		mChatService.setHandler(btHandler);
		
		mIntMaze = BluetoothEncoderDecoder.decodeMaze(getIntent().getStringExtra(KEY_MAZE));
		
		Display display = getWindowManager().getDefaultDisplay();
        Point size = new Point();
        display.getSize(size);
        mDrawView = new DrawView2(this,size.x,size.y,mIntMaze,mHandler);
	...
	//No onOpponentLoss() as opponent's loss is determined by your winning only.
	//In case you win he is informed by the onWin() function
	public void onOpponentCrash(){
		eventCommunicated=true;//to prevent us from sending another message back
		Toast.makeText(this, "Victory!", Toast.LENGTH_LONG).show();
		mDrawView.setDrawState(DrawView2.STATE_WIN);
	}
	public void onOpponentWin(){
		eventCommunicated = true;//not necessary as STATE_LOSS has no callback mechanism
		mDrawView.setDrawState(DrawView2.STATE_LOSS);
	}
	//on<Event> functions below are expected to be called only by mDrawView
	public void onCrash(){
		//no need to update mDrawView's drawState as it alreayd knows when this function is called
